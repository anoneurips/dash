...
ep = 0.05
alpha = 0.8
val_list = [(60,2,.7), (75,3,0.7), (80,4,0.7), (100,4, 0.7)]
...
def dash(k, r, ep, OPT, X, alpha, debug=True, parallel=False):
    ...
    for i in range(r):
        ...
        while ((fST - fS) < (ep/20)*(OPT - fS)) and (len(union(S,T)) < k) and len(X)>0:
            
            # FILTER Step
            # this only changes X
            vs = estimateSet(X, union(S,T), k, r, m)
            while (vs < alpha**(2)*(1-ep)*(OPT - fST)/r) and (len(X) > 1):
